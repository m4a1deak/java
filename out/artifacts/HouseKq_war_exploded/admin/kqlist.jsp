...
<script type="text/javascript">
        ...
        function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/pbmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/pbmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/pbmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function del(id){
			window.location.href="<%=path %>/kqadd.action?id="+id+"&types=1";
		}
		function del2(id){
			window.location.href="<%=path %>/kqadd.action?id="+id+"&types=2";
		}
		</script>
...
<form id="memberForm">
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=path %>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">考勤查询 </SPAN><SPAN 
   ...
   姓名<input type="text" id="realname" name="realname">   开始时间<input type="text" id="btimes" name="btimes" onClick="SelectDate(this.name)"> 结束时间<input type="text" id="etimes" name="etimes" onClick="SelectDate(this.name)">
   <A class=cmdField 
      ...
      border=1>
			...
			for(int i = 0;i<list.size();i++){
				Pb bean = (Pb)list.get(i);
			%>
                
                <TR>
                  <TD class=gridViewItem><%=bean.getNo()%></TD>
                   <TD class=gridViewItem><%=bean.getRealname()%></TD>
                    <TD class=gridViewItem><%=bean.getTimes()%></TD>
                     <TD class=gridViewItem><%=bean.getStates()%></TD>
                </TR>
                <%} %>
              </TBODY>
            </TABLE>
