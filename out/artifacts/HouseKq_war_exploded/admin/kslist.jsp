...
<script type="text/javascript">
        ...
        function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/ksmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/ksmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/ksmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function modify(id){
			window.location.href="<%=path %>/ksmodifybefore.action?id="+id;
		}
		function bmyg(id){
			window.location.href="<%=path %>/bmygmana.action?bmid="+id;
		}
		function del(id){
			window.location.href="<%=path %>/ksdel.action?id="+id;
		}
		</script>
...
<form id="memberForm">
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=path %>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">科室列表 </SPAN><SPAN 
   ...
   科室名<input type="text" id="names" name="names">
   <A class=cmdField 
      ...
      border=1>
               <TBODY>
                <TR>
                  <TH class=gridViewHeader scope=col >科室名</TH>
			...
			%>
                ...
                <TR>
                  <TD class=gridViewItem><%=bean.getNames()%></TD>
