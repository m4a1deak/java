...
function add(){
   if(document.userLogin.password.value=="")
   {
    ...
    if(document.userLogin.newpwd.value=="")
   {
       alert("请输入新密码");
       return;
   }
    if(document.userLogin.newpwd2.value=="")
   {
       alert("请输入确认新密码");
       return;
   }
   if(document.userLogin.newpwd2.value!=document.userLogin.newpwd.value)
   {
       alert("两次输入密码不一致");
       return;
   }
...
<DIV>
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=path %>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">上机密码修改 </SPAN><SPAN 
    ...
    vAlign=top align=middle>
          <DIV><form action="<%=path %>/stucardpwdupdate.action" method="post" name="userLogin">
            <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all 
      border=1>
              <TBODY>
                <TR>
                  <TD class=gridViewItem>原密码</TD> <TD class=gridViewItem><input type="text" id="password" name="password"></TD>
                </TR>
                <TR>
                  <TD class=gridViewItem>新密码</TD> <TD class=gridViewItem><input type="text" id="newpwd" name="newpwd"></TD>
                </TR>
                <TR>
                  <TD class=gridViewItem>确认新密码</TD> <TD class=gridViewItem><input type="text" id="newpwd2" name="newpwd2"></TD>
                </TR>
                 <TR>
