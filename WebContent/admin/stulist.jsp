...
<form id="memberForm">
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=path %>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">学生列表 </SPAN><SPAN 
   ...
   <A class=cmdField 
      ...
      border=1>
               <TBODY>
                <TR>
                  <TH class=gridViewHeader scope=col width="10%">学号</TH>
                  <TH class=gridViewHeader scope=col width="10%">用户名</TH>
                  <TH class=gridViewHeader scope=col width="10%">密码</TH>
                  <TH class=gridViewHeader scope=col width="10%">卡号</TH>
                  <TH class=gridViewHeader scope=col width="10%">卡密码</TH>
                  <TH class=gridViewHeader scope=col width="10%">姓名</TH>
                  <TH class=gridViewHeader scope=col width="10%">电话</TH>
                  <TH class=gridViewHeader scope=col width="8%">余额</TH>
                  <TH class=gridViewHeader scope=col width="7%">时长</TH>
                  <TH class=gridViewHeader scope=col width="5%">状态</TH>
                  <TH class=gridviewHeader scope=col width="5%">修改</TH>
                  <TH class=gridviewHeader scope=col width="5%">删除</TH>
                </TR>
			...
			%>
                ...
                <TR>
                  <TD class=gridViewItem><%=bean.getStuno() %></TD>
                    <TD class=gridViewItem><%=bean.getUsername() %></TD>
                      <TD class=gridViewItem><%=bean.getPassword() %></TD>
                        <TD class=gridViewItem><%=bean.getCardno() %></TD>
                          <TD class=gridViewItem><%=bean.getCardpassword() %></TD>
                            <TD class=gridViewItem><%=bean.getRealname() %></TD>
                              <TD class=gridViewItem><%=bean.getTel() %></TD>
                                <TD class=gridViewItem><%=bean.getYe() %></TD>
                                  <TD class=gridViewItem><%=bean.getSc() %></TD>
                                  <TD class=gridViewItem><%=bean.getCardstates() %></TD>
