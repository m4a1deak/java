...
<script type="text/javascript">
        ...
        function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/dkqmana2.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/dkqmana2.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/dkqmana2.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function del(id){
			window.location.href="<%=path %>/kqadd2.action?id="+id+"&types=1";
		}
		function del2(id){
...
<form id="memberForm">
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=path %>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">待签到列表 </SPAN><SPAN 
      ...
      border=1>
               <TBODY>
                <TR>
                  ...
                  <TH class=gridViewHeader scope=col width="10%">签到</TH>
                </TR>
